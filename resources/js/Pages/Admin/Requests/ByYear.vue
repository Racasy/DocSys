...
function fetchData() {
  router.get(route('admin.users.years.requests', { 
    user: props.user.id, 
    year: props.year 
  }), {
  ...
  <AdminLayout>
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <!-- Header with context -->
        <div class="px-6 py-5 border-b border-gray-200 bg-gray-50 flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 class="text-2xl font-bold text-gray-800">
              {{ user.name }} - {{ year }}
            </h1>
            <p class="text-sm text-gray-600 mt-1">
              Kopā pieprasījumi: {{ requests.length }}
            </p>
          </div>
          <Link 
            href="/admin/requests" 
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Izveidots</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr v-if="requests.length === 0">
                <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">
                  Nav atrasts neviens pieprasījums
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <!-- Delete Confirmation Modal (Same as All.vue) -->
